...
class StandardScaler(object):
    ...
    def transform(self, x):
        standardized = (np.array(x) - self.min) / (self.max - self.min + 1e-8)
    ...
    def fit_invert(self, x, encoded_col):
        self.max, self.min = np.nanmax(x), np.nanmin(x)
        scale = self.max - self.min + 1e-8  # Compute once
        res = np.multiply(encoded_col, scale) + self.min  # Element-wise multiplication
        # res = np.where(encoded_col < 1e-7, np.nan, res)
        return res
...
class DataFrameParser(object):
    ...
    def fit(self, dataframe, threshold):
        ...
        for column, datatype in tqdm(column_to_dtype.items()):
    ...
    def transform_missing(self,missing_indicator):
        df = missing_indicator[self._column_order].copy()
        for column, encoder in self.encoders.items():
            if column in self.numerical_columns:
                df[column] = encoder.fit_transform(df[column].astype(int))
    ...
    def datatype_info(self): 
        print(f"Number of bins:{self.n_bins}, Number of nums: {self.n_nums}, Number of cats:{self.n_cats}")
...
def convert_to_tensor(org_df, gen_output, threshold, data_size, seq_len):
    ...
    # n_paddings = datatype_info['n_paddings'];
    ...
    if n_nums != 0:
        ...
        if 'missings' in gen_output:
            missing_tensor = torch.empty(data_size, seq_len, num_tensor.shape[2]).to(device)
            for idx in range(num_tensor.shape[2]):
                missing_tensor[:,:,idx] = sigmoid_threshold(gen_output['missings'][:,:,idx].detach()).to(torch.int64)
            num_tensor = num_tensor.masked_fill(missing_tensor>0, float('nan'))
        else: 
            missing_tensor = torch.empty(data_size, seq_len, synth_data.shape[2]).to(device)
        synth_data = torch.cat((synth_data, num_tensor),dim=2)
    
    if 'times' in gen_output:
        time_info = torch.empty(data_size, seq_len, 8).to(device)
        time_info = gen_output['times'].detach().to(device)
    else: 
        time_info = torch.empty(data_size, seq_len, 8).to(device)

    if 'eos' in gen_output:
        eos = gen_output['eos'].detach().to(device)
...
def convert_to_table(org_df, synth_data, threshold):
    ...
    for col, bin_col in tqdm(bin_column_pairs,total=len(bin_column_pairs)):
    ...
    for col, cat_col in tqdm(cate_column_pairs,total=len(cate_column_pairs)):
        counts = real_df[col].value_counts()
        repeated_entries = counts[counts > threshold * len(real_df[col])].index.tolist()
    ...
    ##### **Revert Numerical Variables to Original Scale**
    for col in parser.numerical_columns:
        syn_df[col] = StandardScaler().fit_invert(org_df[col], syn_df[col])

    orig_column = org_df.columns
    syn_df = syn_df.reindex(columns=orig_column)    
    feat_num = syn_df.shape[1]
    
    return syn_df, torch.tensor(syn_df.values).reshape(B, L, feat_num)