...
test_ehr_dataset = pickle.load(open('data/datasets/hiv/testDataset.pkl', 'rb'))
test_ehr_dataset = [(p,0) for p in test_ehr_dataset]
synthetic_ehr_dataset = pickle.load(open(args.load_path,'rb'))
synthetic_ehr_dataset = [p for p in synthetic_ehr_dataset if len(p['visits']) > 0]

attack_dataset_pos = list(random.sample(train_ehr_dataset, NUM_TOT_EXAMPLES)) #member
attack_dataset_neg = list(random.sample(test_ehr_dataset, NUM_TOT_EXAMPLES)) # non-member
...
ds = [(find_hamming(ehr, synthetic_ehr_dataset), l) for (ehr, l) in tqdm(test_attack_dataset)] # find distance to synthetic
median_dist = np.median([d for (d,l) in ds]) # decision threshold
preds = [1 if d < median_dist else 0 for (d,l) in ds] # distance vs threshold to predict membership
...
pickle.dump(results, open(f"results/privacy_evaluation/{args.model}_hamming_model.pkl", "wb"))
print(results)


if os.path.isfile(f'./save/halo_model'):
    ...
    pickle.dump(results, open(f"results/privacy_evaluation/{args.model}_perplexity_model.pkl", "wb"))
    print(results)