...
class TextConditionProjector(nn.Module):
    """
    MeLD: the trainable half of text conditioning. The frozen CLIP encoder lives outside
    the DiT (models/text_encoder.py); this maps its pooled embeddings to the DiT hidden
    size and substitutes a learned null embedding for unconditional / CFG-dropped rows.
    """
    def __init__(self, cond_dim, hidden_size, dropout_prob=0.1):
        super().__init__()
    ...
    def forward(self, y, uncond=None):
        """
        y:      (N, cond_dim) pooled text embeddings
        uncond: (N,) bool, True where the row is unconditional; None -> all conditional
        """
        batch_size = y.shape[0]
        if uncond is None:
            uncond = torch.zeros(batch_size, dtype=torch.bool, device=y.device)
        if self.training and self.dropout_prob > 0:
            uncond = uncond | (torch.rand(batch_size, device=y.device) < self.dropout_prob)

        embeddings = self.null_embed.unsqueeze(0).expand(batch_size, -1).clone()
        cond_indices = (~uncond).nonzero(as_tuple=False).flatten()
        if cond_indices.numel() > 0:
            embeddings[cond_indices] = self.projection(y[cond_indices].float()).to(embeddings.dtype)
        return embeddings
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        # MeLD: CLIP is not a submodule (initialize_weights would re-init it); callers pass
        # pooled embeddings from models/text_encoder.py.
        self.y_embedder = TextConditionProjector(cond_dim, hidden_size, dropout_prob=class_dropout_prob)
    ...
    def forward(self, x, t=None, y=None, uncond=None):
        """
        Forward pass of LightningDiT.
        x: (N, C, L) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        y: (N, cond_dim) pooled text embeddings, or None for all-unconditional
        uncond: (N,) bool, True where a row is unconditional; None -> none are
        ...
        if y is None:   # unconditional generation: every row maps to null_embed
            y = x.new_zeros(x.shape[0], self.y_embedder.projection[0].in_features)
            uncond = torch.ones(x.shape[0], dtype=torch.bool, device=x.device)
        y = self.y_embedder(y, uncond)           # (N, D)
        c = t + y                                # (N, D)
    ...
    def forward_with_cfg(self, x, t, y, uncond, cfg_scale, cfg_interval=None, cfg_interval_start=None):
        """
        ...
        y = torch.cat([y[:n], y[:n]], dim=0)
        uncond = torch.cat([uncond[:n], torch.ones_like(uncond[:n])], dim=0)
        model_out = self.forward(combined, t, y, uncond)
