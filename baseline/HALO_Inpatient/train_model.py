...
# val_ehr_dataset = pickle.load(open('./data/hiv/valDataset.pkl', 'rb'))
from sklearn.model_selection import train_test_split
train_ehr_dataset, val_ehr_dataset = train_test_split(train_ehr_dataset, test_size=0.1, random_state=4, shuffle=True)
...
if os.path.exists(f"./save/halo/{args.model}_model"):
  print("Loading previous model")
  checkpoint = torch.load(f'./save/halo/{args.model}_model', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
  ...
  for i in range(0, len(train_ehr_dataset), config.batch_size):
    ...
    if i % (5*config.batch_size) == 0:
      print("Epoch %d, Iter %d: Training Loss:%.6f"%(e, i, loss * 8))
      with open(f"./save/halo/{args.model}_loss","w") as file:
        file.write("Epoch %d, Iter %d: Training Loss:%.6f"%(e, i, loss * 8))
    if i % (5*config.batch_size) == 0:
      if i == 0:
        continue
      ...
      with torch.no_grad():
        ...
        with open(f"./save/halo/{args.model}_loss","w") as file:
          file.write("Epoch %d Validation Loss:%.7f"%(e, cur_val_loss))
