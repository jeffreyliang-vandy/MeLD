...
class HALOModel(nn.Module):
    ...
    def sample(self, input_visits, random=True):
        ...
        #### Softmax interval bit:

        # next_logits = self.ehr_head.sample(hidden_states, input_visits)
        # interval_probs = softmax(next_logits[:,:,0:45])
        # idx_interval = interval_probs.squeeze(1).multinomial(num_samples=1).squeeze(1).cpu().detach().numpy()
        # continuous_bits = torch.zeros_like(interval_probs)
        # for k in range(len(idx_interval)):
        #     continuous_bits[k,0,idx_interval[k]] = 1.0
        # input_visits[:,-1,0:45] = continuous_bits.squeeze(1)
        # i = 45

        # next_logits = self.ehr_head.sample(hidden_states, input_visits)
        # cont_probs = softmax(next_logits[:,:,0:30])
        # if random:
        #     # Multinomial sampling directly in a vectorized manner
        #     idx_cont = cont_probs.squeeze(1).multinomial(num_samples=1).unsqueeze(-1)
        # else:
        #     # Use argmax to select the index with the highest probability
        #     idx_cont = cont_probs.argmax(dim=2, keepdim=True)
        # continuous_bits = torch.zeros_like(cont_probs).scatter_(2, idx_cont, 1.0)
        # input_visits[:, -1, 0:30] = continuous_bits.squeeze(1)
        # i = 30

        # #### Softmax continuous bit:
        # cont_index = [(45,76),(76,107),(107,158),(158,218),(218,225),(225,242)]
        # ##            weight, height,   cd4     , rna_v   ,whostage , cdcstage, death
        # for i_start, i_end in cont_index:

        #     # Missing indicator
        #     next_logits = self.ehr_head.sample(hidden_states, input_visits)
        #     next_probs = sig(next_logits[:, :, i_start])
        #     missing_bits = torch.bernoulli(next_probs) if random else torch.round(next_probs)
        #     input_visits[:, -1, i_start] = missing_bits.squeeze(1)
        #     # Apply softmax over logits slice
        #     next_logits = self.ehr_head.sample(hidden_states, input_visits)
        #     cont_probs = softmax(next_logits[:, :, i_start + 1:i_end])
        #     if random:
        #         idx_cont = cont_probs.squeeze(1).multinomial(num_samples=1).unsqueeze(-1)
        #     else:
        #         idx_cont = cont_probs.argmax(dim=2, keepdim=True)
        #     continuous_bits = torch.zeros_like(cont_probs).scatter_(2, idx_cont, 1.0)
        #     mask = missing_bits.squeeze(1) == 0
        #     # Update `input_visits` with the masked continuous bits
        #     input_visits[mask, -1, i_start + 1:i_end] = continuous_bits[mask].squeeze(1)
        #     # Set `i` to the end of the current range and break out of the `for` loop
                    
        # i = i_end

        while i < self.ehr_head.tot_vocab:
            
            ## For those not in softmax range, use autoregressive
            next_logits = self.ehr_head.sample(hidden_states, input_visits)
