...
class SamplePackingTrainer(Trainer):
    ...
    def get_eval_dataloader(
        ...
        # FIX 2: Force drop_last=True if using DDP (world_size > 1).
        # Uneven batch counts across GPUs during eval causes Deadlock.
        drop_last = self.args.dataloader_drop_last
        if self.args.world_size > 1:
            LOG.info("DDP detected: Forcing drop_last=True for evaluation to prevent deadlock.")
            drop_last = True
