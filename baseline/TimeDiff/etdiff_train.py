...
COLUMNS_DICT = {
    ...
    "hiv": {"numerical": list(range(7)), "categorical": list(range(7,7+141)), "categorical_num_classes": [2 for _ in range(141)]},
}
...
def parse_arguments():        
    ...
    prs.add_argument(
        "--load_path", type=is_file_on_disk, default="preprocess/data_pack/hiv32/train.pt", # required=True,
        help="path to load data"
    )
    prs.add_argument(
        "--data_name", type=str, default="hiv",
    ...
    prs.add_argument(
        "--cut_length", type=int, default=120,
    ...
    prs.add_argument(
        "--model", type=str, default="lstm", choices=["lstm", "gru",'transformer'],
    ...
    prs.add_argument(
        "--batch_size", type=is_positive_integer, default=512, 
        help="batch size"
    )
    prs.add_argument(
    ...
    prs.add_argument(
        "--num_steps", type=is_positive_integer, default=20000, 
    ...
    prs.add_argument(
        "--timesteps", type=is_positive_integer, default=100, 
        help="timesteps for diffusion"
    )
    prs.add_argument(
        "--auto_normalize", action='store_true', default=False, 
    ...
    prs.add_argument("--project_name", type=str, default="hiv-timediff",
        help="Project name for wandb"
    )
    prs.add_argument("--entity", type=str, default='jeffrey-liang-vanderbilt-university',
    ...
    prs.add_argument(
        "--diff_type", type=str, default="gaussian", choices=["mixed", "gaussian","autodiff"],
    )
    prs.add_argument(
...
def model_path_to_sample_path(model_path) -> str:
    sample_path = model_path.replace(".pth", "_samples.npy")
    sample_path = sample_path.replace(".pt", "_samples.npy")
    sample_path = sample_path.replace("models", "samples")
    if sample_path == model_path:
        raise ValueError("Sample Path is the same as Model Path")
    return sample_path
...
if __name__ == "__main__":
    ...
    # if args.model == 'transformer': args.bidirectional = False
    ...
    dataset = TimeSeriesDataset(
        categorical_cols = COLUMNS_DICT[args.data_name]["categorical"] if args.data_name in ["eicu", "mimiciv", "mimiciii", "hirid","hiv"] else None,             # indicate columns that are not time series values (missing indicators)
    ...
    # elif args.diff_type == 'autodiff':
    #     model = BiRNN_score(
    #         input_size=dataset.channels,
    #         hidden_size= args.hidden,
    #         num_layers=3,
    #         diffusion_steps=args.timesteps,
    #         dropout= args.dropout
    #     )
    #     diffusion = GaussianDiffusion(
    #         model = model,
    ...
    if args.eval == False:
        if os.path.exists(args.check_point_path):
            etdiff.load(args.check_point_path)
        ...
        samples = collect_samples(model = etdiff.ema.ema_model, num_samples = 40000, batch_size = 1024, min = etdiff.sample_min, max = etdiff.sample_max)
        ...
        samples = collect_samples(
            ...
            num_samples = 40000, batch_size = 1024, 
