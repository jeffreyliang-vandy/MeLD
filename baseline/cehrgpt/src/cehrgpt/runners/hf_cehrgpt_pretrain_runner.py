...
def load_and_create_model(
    ...
    try:
        ...
        model_config = CEHRGPTConfig(
            ...
            use_cache=cehrgpt_args.use_cache,
...
def main():
    ...
    if training_args.world_size > 1 and "train" in processed_dataset:
        total_len = len(processed_dataset["train"])
        remainder = total_len % training_args.world_size
        # 1. Shuffle to prevent length-based load imbalance (one GPU getting all long sequences)
        LOG.info("DDP: Shuffling training dataset to balance sequence lengths across GPUs...")
        processed_dataset["train"] = processed_dataset["train"].shuffle(seed=training_args.seed)
        # 2. Drop the remainder so both GPUs get the exact same number of input rows
        if remainder != 0:
            LOG.info(f"DDP: Truncating {remainder} samples from training set to make it divisible by world_size ({training_args.world_size}).")
            processed_dataset["train"] = processed_dataset["train"].select(range(total_len - remainder))
    # ======================
