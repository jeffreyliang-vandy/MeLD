...
def generate_single_batch(
    ...
    max_length: int,
    accelerator: Accelerator,  # Pass accelerator to use autocast
    ...
    # We use the model directly since we did NOT use accelerator.prepare(model)
    # This avoids the DDP wrapper overhead.
    
    with torch.no_grad():
        # Accelerator autocast handles fp16/bf16 if configured in 'accelerate config'
...
def main(args):
    ...
    if accelerator.is_main_process:
        LOG.info(f"Accelerator initialized. Device: {device}, Num processes: {accelerator.num_processes}")
    ...
    # CRITICAL CHANGE: Do NOT use accelerator.prepare(model) for inference.
    # Manually move model to the correct device for this process.
    cehrgpt_model = cehrgpt_model.to(device)
    cehrgpt_model.eval()
    ...
    # Access config directly since model is not wrapped
    max_seq_allowed = (
    ...
    # Distribute workload
    patients_per_process = math.ceil(args.num_of_patients / accelerator.num_processes)
    num_of_batches = math.ceil(patients_per_process / args.batch_size)
    
    sequence_to_flush = []
    current_person_id = 1 + (accelerator.process_index * patients_per_process)
    prompt_size = 2 if args.meds_format else START_TOKEN_SIZE

    for i in range(num_of_batches):
        if accelerator.is_main_process:
            LOG.info(f"{datetime.datetime.now()}: Batch {i}/{num_of_batches} started")
        ...
        while len(random_prompts) < args.batch_size:
            indices = random.sample(range(total_rows), k=args.batch_size)
            selected_rows = dataset.select(indices)
            ...
            for row in selected_rows:
                ...
                if len(random_prompts) >= args.batch_size:
                    break
            
            iter_count += 1
            if not random_prompts and iter_count > 10:
                raise RuntimeError(
                    f"The length of concept_ids in {args.demographic_data_path} does not qualify!"
                )

        random_prompts = random_prompts[: args.batch_size]
        random_prompt_ages = random_prompt_ages[: args.batch_size]

        batch_sequences = generate_single_batch(
            cehrgpt_model,
            cehrgpt_tokenizer,
            torch.tensor(random_prompts),
            max_length=args.context_window,
            accelerator=accelerator, # Pass accelerator
            ages=torch.tensor(random_prompt_ages),
